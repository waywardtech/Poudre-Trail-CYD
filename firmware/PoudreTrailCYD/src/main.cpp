...
static void renderTitle(const Snapshot& s){ clearButtons(); tft.fillScreen(bg); tft.setTextColor(accent,bg); tft.setTextDatum(MC_DATUM); tft.drawString("Poudre Trail",160,34,4); tft.drawString("Poudre Valley Edition",160,68,2); tft.setTextDatum(TL_DATUM); tft.setTextColor(textCol,bg); drawWrapped("A valley under pressure.",24,100,270,16); fillButton(btnNewGame,56,172,92,34,"New Game"); fillButton(btnLoadSave,172,172,92,34,"Load Save"); drawButton(btnNewGame,0x2A69,accent); drawButton(btnLoadSave,0x2A69,accent); }
